...
def haversine(lat1, lon1, lat2, lon2):
    return optimizer.haversine(lat1, lon1, lat2, lon2)


def bearing(lat1, lon1, lat2, lon2):
    return optimizer.bearing(lat1, lon1, lat2, lon2)
...
async def create_project(
    ...
    if modo_frota not in {"minimizar", "fixa"}:
        raise HTTPException(422, "modo_frota deve ser 'minimizar' ou 'fixa'.")
    if modo_frota == "fixa" and quantidade_rotas <= 0:
        raise HTTPException(422, "Informe a quantidade de rotas para o modo de frota fixa.")
    if not (0 < meta_ocupacao <= 1):
...
def auto_order_route(project_id: str, route_id: int, tipo: str = "Entrada"):
    ...
    destino = project["destino"]
    ordered = optimizer.nearest_neighbor_order(rows, destino, lat_key, lon_key)
...
def preview_route(project_id: str, route_id: int, tipo: str = "Entrada"):
    ...
    route_coords, distancia_km, duracao_min, used_ors = optimizer.ors_route(coords)
    waypoints = [
    ...
    return {
        ...
        "duracaoMin": round(duracao_min, 1),
    }
...
def gerar_kml(nome_rota, tipo, rows, destino):
    ...
    route_coords, _distancia_km, _duracao_min, _used_ors = optimizer.ors_route(coords)
